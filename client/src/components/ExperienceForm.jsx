...
const ExperienceForm = ({data, onChange}) => {
    ...
    const generateDescription = async (index) => {
        ...
        try {
            const {data} = await api.post('api/ai/enhanc-job-sum', {userContent: prompt}, {headers: {Authorization: token}})
                ...
                {data.map((experience, index)=>(
                    <div key={index} className='p-4 border border-gray-200 rounded-lg space-y-3'>
                        ...
                        <div className='grid md:grid-cols-2 gap-3'>
                            ...
                            {/* <input value={experience.end_date || ""} onChange={(e)=> updateExperience(index, "end_date", e.target.value)} type="month" disabled={experience.is_current === true || experience.is_current === 'true'} className='px-3 py-2 text-sm rounded-lg disabled:bg-gray-100' /> */}
                        ...
                        <label className='flex items-center gap-2'>
                            ...
                            {/* <input type="checkbox" checked={experience.is_current === true || experience.is_current === 'true'} onChange={(e)=>{updateExperience(index, "is_current", e.target.checked);}} className='rounded border-gray-300 text-blue-600 focus:ring-blue-500' /> */}
                        ...
                        <div className='space-y-2'>
                            <div className='flex items-center justify-between'>
                                ...
                                <button onClick={()=> generateDescription(index)} disabled={generatingIndex === index || !experience.position || !experience.company} className='flex items-center gap-1 px-2 py-2 text-xs bg-purple-100 text-purple-700 rounded hover:bg-purple-200 transition-colors disabled:opacity-50'>
